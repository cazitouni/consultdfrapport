...
def fetch_parcelle_data(parcelle, token):
    ...
    locaux_data = requests.get(f'https://apidf-preprod.cerema.fr/ff/locaux?code_insee={parcelle[0:5]}&fields=all&idprocpte={parcelle_data["features"][0]["properties"]["idprocpte"]}', headers=headers, timeout=10).json()
