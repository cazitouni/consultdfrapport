...
def fetch_map(geom, id):
    ...
    wkt = (
        ...
        + ", ".join(
            "("
            + ", ".join(
                "(" + ", ".join("{} {}".format(x, y) for x, y in poly) + ")"
                for poly in ring
            )
            + ")"
            for ring in geom
    ...
    bounds = [
        min(x for coords in geom for poly in coords for x, y in poly),
    ...
    new_bounds = [
        center_x - adjusted_bbox_width / 2,
        center_y - adjusted_bbox_height / 2,
        center_x + adjusted_bbox_width / 2,
        center_y + adjusted_bbox_height / 2,
    ]
    ...
    request_url = (
        qgis_request_url + "&" + "&".join([f"{k}={v}" for k, v in params.items()])
    )
...
def create_pdf(parcelle, proprios, locaux):
    map_data = fetch_map(
        parcelle["geometry"]["coordinates"], parcelle["properties"]["dnupla"]
    )
    title_paragraph = Paragraph(
        f"Relevé Parcellaire - Parcelle {parcelle['id']}", title_style
    )
    ...
    parcelle_table_data = [
        ...
        [
            "Date de l'acte",
            (
                f"{parcelle['properties']['jdatat'][:2]}/{parcelle['properties']['jdatat'][2:4]}/{parcelle['properties']['jdatat'][4:]}"
                if parcelle["properties"]["jdatat"]
                else None
            ),
        ],
        ...
        [
            ...
            " ".join(
                filter(
                    None,
                    [
                        parcelle["properties"].get("dnuvoi"),
                        parcelle["properties"].get("dindic"),
                        parcelle["properties"].get("cconvo"),
                        parcelle["properties"].get("dvoilib"),
                    ],
                )
            )
            .title()
        ...
        [
            "Urbaine",
            "Oui" if parcelle["properties"]["gurbpa"].lower() == "u" else "Non",
        ],
    ]
    ...
    proprietaires_table_data = [
        ["Code", "Type", "Propriétaire", "Adresse", "Date de naissance", "Né.e"]
    ]
    for prop in proprios:
        ...
        nomnss = (
            (
                ...
                f"{((prop.get('ddenom') or '').split('/')[-1].strip())}"
                if (prop.get("dnomus") or "").strip()
                != (prop.get("dnomlp") or "").strip()
                else ""
            )
            .strip()
            .capitalize()
        )
        proprietaires_table_data.append(
            [
                P(prop["dnuper"]),
                P(prop["ccodro"]),
                P(nomprop),
                P(adrprop.title()),
                (
                    P("Le " + prop["jdatnss"] or +" à " + prop["dldnss"])
                    if prop["jdatnss"]
                    else ""
                ),
                P(nomnss),
    ...
    proprietaires_table = Table(
        proprietaires_table_data, colWidths=[62, 90, 90, 130, 100, 90], repeatRows=1
    )
    proprietaires_table.setStyle(style)
    if len(proprios) < 3:
        content.extend(
            [Spacer(1, 20), title_prop, Spacer(1, 5), proprietaires_table, PageBreak()]
        )
    else:
        content.extend(
            [
    ...
    for local in locaux:
        numeroinvar = (
            f"{(local.get('invar') or '')[3:] + ' ' + (local.get('cleinvar') or '')}"
        ).strip()
        ndevoirie = (
            f"{str(local.get('dnvoiri') or '').lstrip('0')}{str(local.get('dindic') or '')}"
        ).strip()
        local_detail_table_data.append(
            [
                numeroinvar,
                local["dpor"],
                local["ccoriv"],
                local["dnubat"] + "-" + local["descc"] + "-" + local["dniv"],
                P(ndevoirie + " " + local["dvoilib"].title()),
                local["cconlc"],
                local["ccoeva"],
                local["gtauom"],
            ]
        )
    local_detail_table = Table(
        local_detail_table_data,
        colWidths=[90, 50, 60, 70, 112, 50, 70, 50],
        repeatRows=1,
    )
